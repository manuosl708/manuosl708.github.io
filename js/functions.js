...
function toggle_start_stop(mthis)
{
    ...
    {
        audio_click.play();
        if(window.confirm("Press OK to stop recording!"))
        {
            clearInterval(recording_interval);      
            running_bo = false;
...
function download_bookmark_json() {
    if(0 < bookmark_dict['Bookmarks'].length)
    {
        ...
        var textStr = "data:text/json;charset=utf-8," + encodeURIComponent(json_to_text(bookmark_dict));
        ...
        json_text.click();
        window.postMessage('Data downloaded!');
    }
    else
    {
        audio_error.play();
        window.confirm('Data is empty - no download!');
    }
}
...
function special_comment() {
    ...
    {
        audio_click.play();
        comment_cstm = prompt("Comment", "Default");   
    }
    else
    {
        audio_error.play();
